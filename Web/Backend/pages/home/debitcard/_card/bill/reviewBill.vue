...
export default {
  ...
  async mounted () {
    ...
    if (this.$route.params.bank !== 'BRB') {
      ...
      if (bankdata.data.status === 200) {
        ...
        this.$router.replace('/home/debitcard/' + this.$route.params.card + '/bill/' + this.$route.params.bank)
      }
    } else {
      this.bankdata = {
        title: 'Bara Bank'
  ...
  methods: {
    async submit () {
      // API
      const result = await this.$axios.post('api/user/debitcard/bill', {
        mode: this.mode,
      ...
      if (result.data.status === 200) {
        this.$router.push('/home/debitcard/' + this.$route.params.card + '/bill/Bill_Slip/' + result.data.data.insertId)
